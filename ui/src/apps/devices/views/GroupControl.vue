<template>
  <div class="details-view group-control-view">
    <emq-details-page-head>
      <el-breadcrumb slot="breadcrumb">
        <el-breadcrumb-item to="/devices/groups">{{ $t('groups.group') }}</el-breadcrumb-item>
        <el-breadcrumb-item>{{ $t('groups.groupControl') }}</el-breadcrumb-item>
      </el-breadcrumb>
    </emq-details-page-head>
    ...
    <emq-crud
      ...
      :url="`/groups/${$route.params.id}/control_logs`"
      :tableActions="[]">
      <template slot="tableColumns">
        ...
        <el-table-column
          v-if="currentGroup.cloudProtocol !== $variable.cloudProtocol.LWM2M"
          width="140px"
          prop="topic"
...
export default {
  ...
  methods: {
    loadRecord() {
      httpGet(`/groups/${this.$route.params.id}`).then((res) => {
        this.currentGroup = res.data
        this.timerUrl = `/timer_publish?groupID=${this.currentGroup.groupID}`
      })
    },
